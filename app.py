...
def table_to_line_items(rows: list[list[str]]) -> pd.DataFrame | None:
    ...
    for i, row in enumerate(rows[:5]):
        ...
        if "Part Number" in hits and len(hits) >= 3:
...
LINE_RE = re.compile(
    ...
    r"[\s|,;]+(?P<qty>\d[\d,]*(?:\.\d+)?)"
    r"[\s|,;]+(?:[$])?(?P<price>\d[\d,]*(?:\.\d+)?)"
    r"[\s|,;]+(?P<date>\d{1,2}[-/ ][A-Za-z]{3,}[-/ ]\d{2,4}|\d{4}-\d{2}-\d{2}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4})",
    re.I,
)
...
def compare_to_master(items: pd.DataFrame, master: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, item in items.iterrows():
        ...
        if not found:
            ...
            if not po_grade:
                issues.append("Missing material grade on PO")
            elif master_grade and po_grade.upper() != master_grade.upper():
                issues.append(
    ...
    if pdf_bytes is None:
        ...
        with st.expander("Expected PO layout"):
            st.markdown(
                ...
                "- Or line text such as `PN-1001 SS304 240 12.50 15-Oct-2026`.\n"
